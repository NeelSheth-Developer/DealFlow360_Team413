...
type Case = {
  name: string;
  input: Parameters<typeof scoreLines>[0];
...
const cases: Case[] = [
  {
    ...
    input: {
      ...
      tier: 'gold',
      orderDiscountPct: 0,
      lines: [L('hardware', 1, 1000, 10), L('service', 1, 2000, 18)],
    },
    expect: {
      score: 5.33,
      worstSingleOverage: 8,
      totalValue: 3000,
      weightedOverage: 16000,
  ...
  },
  {
    ...
    input: {
      ...
      tier: 'gold',
      orderDiscountPct: 0,
      lines: [L('hardware', 8, 87400, 12), L('service', 1, 18400, 18)],
    },
    expect: {
      score: 0.21,
      worstSingleOverage: 8,
  ...
  },
  {
    ...
    input: {
      ...
      tier: 'gold',
      orderDiscountPct: 0,
      lines: [L('service', 2, 5000, 30)],
    },
    expect: {
      score: 20,
      worstSingleOverage: 20,
      totalValue: 10000,
      approvers: 'sales_manager,finance',
    },
  },
...
for (const c of cases) {
  ...
  const actual: Record<string, unknown> = {
    score: r.score,
    worstSingleOverage: r.worstSingleOverage,
    totalValue: r.totalValue,
    weightedOverage: r.weightedOverage,
    approvers: r.approvers.join(','),
    givenPct: r.lineBreakdown[0]?.givenPct,
    overBy: r.lineBreakdown[0]?.overBy,
  };
  ...
  if (bad.length) {
    ...
    for (const [k, v] of bad)
      console.log(`        ${k}: expected ${String(v)}, got ${String(actual[k])}`);
  }
}
